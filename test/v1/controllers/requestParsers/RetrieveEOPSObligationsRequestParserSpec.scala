...
class RetrieveEOPSObligationsRequestParserSpec extends UnitSpec {
  ...
  val status                               = "Open"
  val convertedStatusOpen: MtdStatus       = MtdStatus.Open
  val convertedStatusFulfilled: MtdStatus  = MtdStatus.Fulfilled
  implicit val correlationId: String       = "a1e8057e-fbbc-47a8-a8b4-78d9f015c253"

  val data: RetrieveEOPSObligationsRawData =
    RetrieveEOPSObligationsRawData(nino, Some(typeOfBusiness), Some(businessId), Some(fromDate), Some(toDate), Some(status))

  val todaysDatesData: RetrieveEOPSObligationsRawData =
    RetrieveEOPSObligationsRawData(nino, Some(typeOfBusiness), Some(businessId), None, None, Some("Fulfilled"))

  val invalidNinoData: RetrieveEOPSObligationsRawData =
    RetrieveEOPSObligationsRawData("Walrus", Some(typeOfBusiness), Some(businessId), Some(fromDate), Some(toDate), Some(status))

  val invalidMultipleData: RetrieveEOPSObligationsRawData =
    RetrieveEOPSObligationsRawData("Walrus", Some("Beans"), Some(businessId), Some(fromDate), Some(toDate), Some(status))
