...
class RetrieveEOPSObligationsValidatorFactorySpec extends UnitSpec with Inside {

  private implicit val correlationId: String = "1234"
  private implicit val clock: MutableClock   = MutableClock.epochUTC()

  val validatorFactory = new RetrieveEOPSObligationsValidatorFactory
    ...
    "only an Open status and no other fields are supplied" must {
      "return the parsed domain object with None in the other fields" in {
        validatorFactory
          ...
          .validateAndWrapResult() shouldBe
          Right(RetrieveEOPSObligationsRequest(Nino(validNino), None, None, None, Some(MtdStatusV3.open)))
      }
    }
