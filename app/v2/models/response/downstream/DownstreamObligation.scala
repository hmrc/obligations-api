...
case class DownstreamObligation(identification: Option[DownstreamIdentification], obligationDetails: Seq[DownstreamObligationDetail]) {
  ...
  def filterObligationDetails(predicate: DownstreamObligationDetail => Boolean): DownstreamObligation =
    copy(obligationDetails = obligationDetails.filter(predicate))
